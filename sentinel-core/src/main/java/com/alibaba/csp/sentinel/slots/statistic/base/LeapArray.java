...
public abstract class LeapArray<T> {
    ...
    private int calculateTimeIdx(/*@Valid*/ long timeMillis) {
        long timeId = timeMillis / windowLengthInMs;//将时间分成500ms的时间段
        // Calculate current index so we can map the timestamp to the leap array.
        return (int)(timeId % array.length());
    }
    //计算窗当前窗口开始的时间  timeMillis % windowLengthInMs 余数为当前窗口逝去了过长时间
    ...
    public WindowWrap<T> currentWindow(long timeMillis) {//timeMillis 当前时间
        ...
        while (true) {
            ...
            if (old == null) {
            ...
            } else if (windowStart == old.windowStart()) {//当前时间窗体开始时间和当前时间计算的开始时间一致，则返回该窗体
                /*
                 *     B0       B1      B2     B3      B4
                 * ||_______|_______|_______|_______|_______||___
                 * 200     400     600     800     1000    1200  timestamp
                 ...
                 */
                return old;
            } else if (windowStart > old.windowStart()) {//当当前时间计算的窗体大于该窗体，则
                /*
                 *   (old)
                 *             B0       B1      B2    NULL      B4
